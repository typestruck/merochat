...
import Shared.Profile.Types (PM, ProfileAsk, ProfileMessage(..), ProfileModel, What(..))
...
setVisibility model modal = model { mode = mode, visible = visible } /\ []
      where
      mode /\ visible = case modal of
            ShowProfile profileMode → profileMode /\ true
            _ → model.mode /\ false
