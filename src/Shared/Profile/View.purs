...
view model = HE.div (show ProfileEditionForm)
      [ HE.div [ HA.class' { "profile-edition": true, hidden: not model.visible } ]
              ...
              , HE.div (HA.class' "profile-section")
                      ...
                      , HE.textarea' [ HA.class' "modal-input", HA.maxlength descriptionMaxCharacters, HA.rows 7, HA.type' "text", HA.value $ DM.fromMaybe "" model.descriptionInputed, onChange (SetAutoGenerated Description) ]
                      ]
              , HE.div (HA.class' "profile-section profile-buttons")
                      [ HE.input [ HA.type' "button", HA.class' "cancel", HA.value "Discard" ]
                      , HE.input [ HA.type' "button", HA.class' "green-button bigger", HA.value "Save" ]
                      ]
              ]
      ]
      where
