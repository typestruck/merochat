...
tests = do
        TU.suite "im suggestion update" do
                TU.test "nextSuggestion sets suggesting to zero if Nothing" do
                        let { suggesting } = DT.fst <<< CIS.nextSuggestion <<< TSU.updateModel model $ _ {
                                suggestions = [imUser],
                                suggesting = Nothing
                        }
                ...
                TU.test "nextSuggestion clears chatting" do
                        let { chatting } = DT.fst <<< CIS.nextSuggestion <<< TSU.updateModel model $ _ {
                                chatting = Just 2,
                                suggestions = [imUser, imUser]
                        }
                ...
                TU.test "previousSuggestion descreases suggesting" do
                        let { suggesting } = DT.fst <<< CIS.previousSuggestion <<< TSU.updateModel model $ _ {
                                suggestions = [imUser, imUser],
                                suggesting = Just 1
                        }
                        TUA.equal (Just 0) suggesting
                ...
                TU.test "displayMoreSuggestions clears chatting" do
                        let { chatting } = DT.fst <<< CIS.displayMoreSuggestions [] <<< TSU.updateModel model $ _ {
                                chatting = Just 2,
                                suggestions = [imUser, imUser]
                        }
                        TUA.equal Nothing chatting