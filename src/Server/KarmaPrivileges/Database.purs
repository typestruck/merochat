...
fetchTop10 = (SD.query $ select (((u ... _id) # as _id) /\ ((u ... _name) # as _name) /\ ((u ... _avatar) # as _avatar) /\ ((k ... _position) # as _position) /\ ((k ... _current_karma) # as _karma)) # from (((karma_leaderboard # as k) `join` (users # as u)) # on (k ... _ranker .=. u ... _id)) # orderBy (k ... _id) # limit (Proxy ∷ _ 10))
...
fetchInBetween10 position = SD.unsafeQuery """select u.id "id", u.name, u.avatar, position, current_karma karma from karma_leaderboard k join users u on k.ranker = u.id where position between greatest(1, @position - 5) and @position + 5 order by position""" { position }
