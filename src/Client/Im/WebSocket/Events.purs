module Client.Im.WebSocket.Events  where
...
startWebSocket = do
      ...
      webSocketStateR ← ER.new  { webSocket , pingId: Nothing, privilegesId: Nothing }
     -- setUpWebSocket webSocketStateRef
      pure webSocketStateR
...
--             status ← WSWS.readyState state.webSocket
...
--       state ← ER.read webSocketStateRef
--       let webSocketTarget = EU.unsafeCoerce state.webSocket
...
--             when focused <<< CIW.sendPayload webSocket $ UpdateAvailability { online: true }
