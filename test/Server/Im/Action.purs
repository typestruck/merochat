...
tests = do
      TU.suite "im actions" do
            ...
            TU.test "suggest respects hidden visibility"
                  ...
                  $ do
                          Tuple userId anotherUserId ← setUpUsers
                          SSA.changePrivacySettings anotherUserId { postsVisibility: Nobody, profileVisibility: Nobody, onlineStatus: true, typingStatus: true, messageTimestamps: true, readReceipts: true }
                          suggestions ← SIA.suggest userId 0 ThisWeek
            ...
            TU.test "suggest respects contacts only visibility"
                  ...
                  $ do
                          Tuple userId anotherUserId ← setUpUsers
                          SSA.changePrivacySettings anotherUserId { postsVisibility: Nobody, profileVisibility: Contacts, onlineStatus: true, typingStatus: true, messageTimestamps: true, readReceipts: true }
                          suggestions ← SIA.suggest userId 0 ThisWeek
                          R.liftAff $ TUA.equal [] suggestions
            ...
            TU.test "suggest respects no temporary users only visibility"
                  ...
                  $ do
                          ...
                          SSA.changePrivacySettings userId { postsVisibility: Nobody, profileVisibility: NoTemporaryUsers, onlineStatus: true, typingStatus: true, messageTimestamps: true, readReceipts: true }
                          suggestions ← SIA.suggest userId 0 ThisWeek
                          R.liftAff $ TUA.equal [] suggestions
            ...
            TU.test "suggest respects no temporary users only visibility when suggesting to temporary user"
                  ...
                  $ do
                          ...
                          SSA.changePrivacySettings anotherUserId {postsVisibility : Nobody, profileVisibility: NoTemporaryUsers, onlineStatus: true, typingStatus: true, messageTimestamps: true, readReceipts: true }
                          suggestions ← SIA.suggest userId 0 ThisWeek
                          R.liftAff $ TUA.equal [] suggestions
            ...
            -- TU.testSkip "listMissedContacts finds missed messages"
            --       $ TS.serverAction
            --       $ do
            --               Tuple userId anotherUserId ← setUpUsers
            --               yetAnotherUserId ← SLD.createUser $ baseUser { email = Just "d@d.com" }
            ...
            --               ev ← SIA.listMissedContacts userId (Just 0) dt
            ...
            -- TU.testSkip "listMissedContacts finds missed messages past date"
            --       $ TS.serverAction
            --       $ do
            --               Tuple userId anotherUserId ← setUpUsers
            --               yetAnotherUserId ← SLD.createUser $ baseUser { email = Just "d@d.com" }
            ...
            --               void <<< SIA.processMessage yetAnotherUserId userId $ Text "hey"
            --               dt ← liftEffect $ EN.nowDateTime
            --               ev ← SIA.listMissedContacts userId Nothing <<< SU.fromJust $ DT.adjust (Days (-1.0)) dt
            --               R.liftAff <<< TUA.equal 3 $ DA.length ev.missedMessages
            --               R.liftAff $ TUA.equal anotherUserId (ev.missedMessages !@ 0).sender
            --               R.liftAff $ TUA.equal "oi" (ev.missedMessages !@ 0).content
            --               R.liftAff $ TUA.equal yetAnotherUserId (ev.missedMessages !@ 1).sender
            ...
            -- TU.test "listMissedContacts ignores delivered messages"
            --       $ TS.serverAction
            --       $ do
            --               Tuple userId anotherUserId ← setUpUsers
            --               yetAnotherUserId ← SLD.createUser $ baseUser { email = Just "d@d.com" }
            ...
            -- TU.testSkip "listMissedContacts syncs messages"
            --       $ TS.serverAction
            --       $ do
            --               Tuple userId anotherUserId ← setUpUsers
            --               yetAnotherUserId ← SLD.createUser $ baseUser { email = Just "d@d.com" }
            ...
            --               void <<< SIA.processMessage userId yetAnotherUserId $ Text "hey"
            --               dt ← liftEffect $ EN.nowDateTime
            --               ev ← SIA.listMissedContacts userId Nothing dt
            --               R.liftAff <<< TUA.equal 3 $ DA.length ev.missedMessages
