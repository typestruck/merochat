...
selectRecoverer token = SD.scalar (Query "select recoverer from recoveries where uuid = $1 and active = true and created >=  current_timestamp - interval '1 day'") $ Row1 token

recoverPassword :: String -> PrimaryKey -> String -> ServerEffect Unit
recoverPassword token id password = SD.withTransaction $ \connection -> void do
