...
fetchInBetween10 position = SD.select (Query "select u.name, u.avatar, position, current_karma karma from karma_leaderboard k join users u on k.ranker = u.id where position >= $1 - 5 and position <= $1 + 5") $ Row1 position