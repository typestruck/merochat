...
$$
begin
    create temporary table temp_karmas (id integer, target interger, amount integer ) on commit drop;
    insert into temp_karmas
    select id,
            target,
            amount
    from karma_histories
    where extract(epoch from (now() at time zone 'utc') - k.date ) / 3600 = hours_time;
...
begin
    ...
    insert into karma_leaderboard (ranker, current_karma, position)
    select  target,
            total,
            row_number() over (order by total desc)
    from (select target, sum(amount) total from karma_histories group by target) a;
end;
$$
language plpgsql;
