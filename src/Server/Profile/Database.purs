...
presentProfile loggedUserID = map SU.fromJust <<< SD.single $ select profilePresentationFields # from source # wher filtered
      ...
      profilePresentationFields = (u ... _id # as _id)
            ...
            /\ _description
            /\ _country
            /\ (select (int_array_agg (l ... _id) # as _languages) # from (((languages # as l) `join` (languages_users # as lu)) # on (l ... _id .=. lu ... _language .&&. lu ... _speaker .=. u ... _id)) # orderBy (l ... _name) # limit 1)
